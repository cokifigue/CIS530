...
class DocumentPrep(object):
    ...
    def extract_input(self):
        ...
        vectorizer=TfidfVectorizer(ngram_range=(1,3),min_df=2,sublinear_tf=True)#stop_words='english',min_df=3)
    ...
    def extract_input_test(self):
        ...
        vectorizer=TfidfVectorizer(ngram_range=(1,3),min_df=2,sublinear_tf=True,tokenizer=word_tokenize)#stop_words='english',min_df=3)
...
if __name__=="__main__":
    ...
    sent_len=doc_prep.sentence_length_count()
    avg_word_len=doc_prep.avg_word_length()
    avg_sent_len=doc_prep.avg_word_in_sent()
...
#     train=transformed_data[:train_len,:]
    ...
    test=data[train_len:,:]
    #POS Tagged text:
    doc_prep_pos=DocumentPrep(indir=local_dir + '/train/train_pos.txt',testdir= local_dir + '/test/test_pos.txt')
    ...
    data_test = add_feature(data_pos, sent_len)
    data_test=add_feature(data_test,avg_word_len)
    data_test=add_feature(data_test, avg_sent_len)
    data_test=hstack([data_test, data]).tocsr()
